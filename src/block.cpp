...
std::vector<vertice> Block::get_vertices(int matrix[X_DIMENSION] [Y_DIMENSION][Z_DIMENSION], Coordinate coordinate, Coordinate chunk_coordinate) {

    int i = 0;
    std::vector<vertice> vertex_list;


    if (matrix[coordinate.x - chunk_coordinate.x][coordinate.y - chunk_coordinate.y][coordinate.z - chunk_coordinate.z] == -1) return vertex_list;
