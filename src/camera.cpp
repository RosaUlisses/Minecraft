    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (glm::length(x_distance) < glm::length(y_distance) && glm::length(x_distance) < glm::length(y_distance)) {
            ...
            if (chunk->is_there_a_block(look_at_block)) {
                return new Coordinate (look_at_block);
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods) {
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        Coordinate* look_at_block = camera->ray_casting();

        if (look_at_block != nullptr) {
            world->destroy_block(*look_at_block);
        }   

        delete look_at_block;
    }
}
